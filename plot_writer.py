...
class PlotWriter(object):
    ...
    def write(self, model):
        ...
        plt.plot(list(range(model.n_train, len(model.data))), model.pred_valid_lower, 'darkred', label='pred_valid_lower')
        plt.plot(list(range(model.n_train, len(model.data))), model.pred_valid_upper, 'salmon', label='pred_valid_upper')
        plt.plot(list(range(len(model.data), len(model.data) + len(model.pred_test))), model.pred_test, 'c', label='pred_test')
        plt.plot(list(range(len(model.data), len(model.data) + len(model.pred_test))), model.pred_test_lower, 'darkcyan',
                 label='pred_test_lower')
        plt.plot(list(range(len(model.data), len(model.data) + len(model.pred_test))), model.pred_test_upper, 'aquamarine',
