...
def train_standard(data_scaled, config):
	...
	# build network
	inputs = keras.layers.Input(shape=(config.n_timestep, config.n_input_dim), batch_shape=(config.n_batch, config.n_timestep, config.n_input_dim))
	grus = []
	denses = []
	...
	for i in range(config.n_max_epoch):
		...
		# because
		loss = model.fit(x_train, y_train, epochs=1, batch_size=config.n_batch, verbose=1, shuffle=False, validation_data=(x_valid, y_valid))
		model.reset_states()

		print('train loss: {}, valid loss: {}'.format(loss.history['loss'][0], loss.history['val_loss'][0]))
		total_loss = loss.history['loss'][0] * (1-config.valid_loss_weight) + loss.history['val_loss'][0] * config.valid_loss_weight
		# push into heap
...
# if more than 50 percent samples are zero, this time series is considered as zero inflated.
if np.count_nonzero(data) < len(data)*0.5:
	print('Zero inflated')
	exit()
