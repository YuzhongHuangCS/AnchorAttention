...
class RNNPredictor(object):
    ...
    def predict(self, content):
        ...
        n_valid = max(min(int(len(x) * self.config.ratio_valid), self.config.max_valid), self.config.n_predict_step)
        ...
        #batchY_placeholder = tf.placeholder(tf.float32, [n_total, 1])
        ...
        #W4 = tf.get_variable('W4', shape=(self.config.n_dense3_dim, self.config.n_output_dim))
        #b4 = tf.get_variable('b4', shape=(1, self.config.n_output_dim), initializer=tf.zeros_initializer())
        ...
        #cell2 = tf.nn.rnn_cell.GRUCell(self.config.n_neurons2)
        ...
        #prediction = tf.matmul(tf.tanh(tf.matmul(tf.tanh(tf.matmul(tf.tanh(tf.matmul(tf.squeeze(states_series), W1) + b1), W2) + b2), W3) + b3), W4) + b4
        ...
        pred_point_test = tf.slice(prediction, (n_train+n_valid-n_predict_step, 0), (n_predict_step, 1))
        pred_lower_test = tf.slice(prediction, (n_train+n_valid-n_predict_step, 1), (n_predict_step, 1))
        pred_upper_test = tf.slice(prediction, (n_train+n_valid-n_predict_step, 2), (n_predict_step, 1))
